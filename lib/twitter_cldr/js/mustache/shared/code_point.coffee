...
class TwitterCldr.CodePoint
	...
	constructor : (@fields) -> # TODO this loop is probably not going to work.
		for i in [0..TwitterCldr.CodePoint.code_point_fields]
	...
	decomposition : ->
		...
		if (match = decomp.match(decomposition_regex) and match[1])
			match[1].match(/\S+/g).map ((s) -> #splitting by whitespace
				parseInt(s, 16)
			)
		else 
	...
	@find : (code_point) ->
		...
		return unless target? and target.first?

		block_data = @block_data[target.first]
		code_point_data = block_data[code_point]
		if !code_point_data?
			code_point_data = @get_range_start(code_point, block_data)
	...
	@for_canonical_decomposition : (code_points) ->
		if @canonical_compositions[code_points]?
	...
	@hangul_type : (code_point) ->
		...
		if code_point
			for type in ["lparts", "vparts", "tparts", "compositions"]
				for range_hash in @hangul_type[type]
	...
	@is_excluded_from_composition : (code_point) ->
		...
		for exclusion in @composition_exclusions
			range = new TwitterCldr.Range (exclusion.start, exclusion.end)
	...
	@get_block : (code_point) ->
		...
		for k, range of @blocks
			...
			if range.includes(code_point)
				return @block_cache[code_point] = range
